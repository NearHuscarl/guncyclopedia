        ...
        {paddedSegments.map(({ value, cappedValue, tooltip = "", isEstimated, color, chance }, i) => {
          ...
          const barProps: HTMLAttributes<HTMLElement> = {
            style: {
              ...
              opacity: isUnknownChance ? 0.15 : Math.max(chance ?? 1, 0.15),
            },
            className: clsx({
              "bg-white transition-all duration-160 ease-out hover:bg-primary! hover:opacity-100!": true,
              "bg-stone-300!": isEstimatedValue && !isInfiniteValue,
